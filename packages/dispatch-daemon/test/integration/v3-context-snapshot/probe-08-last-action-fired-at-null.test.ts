/**
 * MB-T10 — Probe P8: last_action_fired_at returns null in v3.0.
 *
 * Per CONDUCTOR_V3_RESCOPE.md §4 line 199 (MB-T10 out of scope —
 * lands in MB-T11). The schema field is `string().datetime().nullable()`
 * (§11). v3.0 daemon hard-codes null.
 *
...
describe('MB-T10 — P8 last_action_fired_at is null in v3.0 (MB-T11 will populate)', () => {
  ...
  it('returns last_action_fired_at: null on every request', async () => {
    const handoffPath = await mkdtempPath('mbt10-p08-ho-', 'HANDOFF.md');
    await writeFile(handoffPath, '# anything\n', 'utf8');
    const registryPath = await mkdtempPath('mbt10-p08-reg-', 'sessions.json');
    ...
    ts = await spawnTestServer({
      tokenPath: await mkdtempPath('mbt10-p08-tok-', 'token'),
      registryPath,
    });
    ...
    const body = (await r.json()) as { last_action_fired_at: string | null };
    expect(body.last_action_fired_at).toBeNull();
  });
});
