...
| `MB-F-COARCH-T02-REAL-DAEMON-WIRING` | Round 2 ChatPanel uses a stub DaemonClient (createStubDaemonClient()) per CRITICAL-1 operator resolution (2026-04-30). Real `/v3/*` HTTP wiring deferred to COARCH-T02b or COARCH-T03 per operator sequencing. When the real client is wired: (1) Zipper-2 exposes `window.coarchitectBridge.fetchHistory()`/`postMessage()` via contextBridge (RESOLUTION-3 Pattern B); (2) a thin adapter maps the bridge calls to the DaemonClient interface; (3) mount.ts auto-mount block replaced with bridge adapter call; (4) V3_TICKETS.md acceptance criterion ("persists to daemon, survives app restart with history intact") becomes testable. OPEN-Q-D-2 (dispatch-core public exports map for ChatMessage ↔ OrchestratorMessageRow import) must be resolved before Zipper-2 swaps in the typed import. | COARCH-T02 green |
...
| `MB-F-ZIPPER-2-COARCH-T03-IPC` | Zipper-2 wires `window.coarchitectBridge` (Pattern B, RESOLUTION-3): `fetchHistory()` and `postMessage()` route to stub DaemonClient. COARCH-T03 must replace the stub with a real HTTP client (`/v3/messages` GET + POST). At that point: (1) replace `createStubDaemonClient()` in `coarchitect-ipc.ts` with a real implementation; (2) mount.ts auto-mount stub block replaced by a bridge-adapter that reads from `window.coarchitectBridge`; (3) integration test for real message persistence added. | Zipper-2 green 039721f |
