...
| `MB-F-COARCH-T04-CONTEXT-HALT` | HALT surface: build-doc-schema-spec.md ticket description says "only the ticket section relevant to the triggering event" (partial injection), while system-prompt.md §"Context-injection at runtime" item 2 says "loaded build doc content as the first user message" (full injection). Current implementation uses full-doc injection matching the frozen system prompt (ratified e6e83f9). Requires operator arbitration before implementing ticket-section filtering. Pending; no current commitment to change. | COARCH-T04 green |
