...
| BUILD.md tab | **SHIPPED-shell (placeholder body; T5 owns content)** (was WIREFRAME-VISION-NOT-SHIPPED at audit-anchor `5704dd2`) | **Closed at `7abb649 green(MB-T-WIREFRAME-T4-BOTTOM-RAIL-CONDUCTOR-CONTROLS): WB13 — mount.ts production wiring` 2026-05-12** (Sub-Q-T4-B=(iii) defer-with-placeholder; T5 ticket `c92f750` owns BUILD.md content rendering). Tab shell SHIPPED: NEW `<button data-testid="chat-shell-tab-build-md">BUILD.md</button>` as third tab in chat-shell-tab-strip via mount.ts resolveTabs() bridge path; clicking renders `<div data-testid="build-md-tab-placeholder">` body with text cross-referencing T5 ownership. NEW `src/chat-shell/build-md-tab.tsx` ships the placeholder component (44 lines). Probes: `test/unit/chat-shell/probe-mbtwt4-03-build-md-tab-shell.spec.tsx` (4 it-blocks; 4/4 GREEN). MB-T23 reference superseded: T5 (`MB-T-WIREFRAME-T5-BUILD-MD-DRIVEN-DISPATCH`) is the current owner of BUILD.md content. Findings doc at `docs/coordination/mbtwt4-findings-2026-05-12.md`. Original-audit evidence preserved: ~~`mount.ts:65`: `// future MB-T23 Tasks tab`. No BUILD.md tab component.~~ [KNOWN — `mount.ts` resolveTabs() + `build-md-tab.tsx`] |
...
**§5 amendment 2026-05-12 (post MB-T-WIREFRAME-T4-BOTTOM-RAIL-CONDUCTOR-CONTROLS WB13 `7abb649`):** BUILD.md tab row (line 164 above) reclassified WIREFRAME-VISION-NOT-SHIPPED → **SHIPPED-shell** (placeholder body per Sub-Q-T4-B=iii; T5 owns content). T4 also ships the broader Dim 3 chrome shipment: NEW Conductor brand label (`bottom-rail-brand` leftmost in chat-shell-header-bar; WB2 `57dc77a`); NEW max-parallel counter component (`max-parallel-counter` slot in header-bar; WB4 `b9e8d4b`; mount.ts auto-wire deferred to Tier 2 `MB-F-T4-BOTTOM-RAIL-MOUNT-WIRING`); NEW wireframe-formatted cost-meter (`bottom-rail-cost-meter` "conductor api · $X.XX today"; WB8 `edb0fba` + WB13 mount.ts swap; aggregation backend stubbed per Tier 3 `MB-F-COST-METER-AGGREGATION-BACKEND-STUBBED`); NEW plan-timer text component (`plan-timer-text` countdown from requests.reset; WB10 `4dc4f32`; mount.ts auto-wire Tier 2); NEW bypass-perms indicator (`bypass-perms-indicator` derived from dispatchMode=auto; WB12 `26ff2c2`; mount.ts auto-wire Tier 2). Slot-prop pattern preserved (mirrors MB-T24/25/26/27); existing chrome zones unchanged. 6 NEW probes + 25 new it-blocks; 72/72 chat-shell suite GREEN. Findings doc at `docs/coordination/mbtwt4-findings-2026-05-12.md`. Mix-indicator stub (MB-F-T27) remains open — sibling territory, not in T4 scope.
...
| **10.3 BUILD.md tab** | BUILD.md tab in Conductor panel showing task list + state classification | **PARTIAL-SHIPPED 2026-05-12** (post MB-T-WIREFRAME-T4 WB13 `7abb649`): tab shell shipped per Sub-Q-T4-B=iii defer-with-placeholder; T5 ticket (`c92f750`) owns BUILD.md content rendering. ~~BUILD.md tab absent (`mount.ts:65` comment: "future MB-T23"). MB-T23 open ticket.~~ | **M** (multi-WB; BUILD.md parser MB-T28 + classifier MB-T29 prerequisites; tab body itself is one WB) | **Tab shell SHIPPED at v3.5.0+T4 (placeholder body); content delivery deferred to T5 ladder.** |
